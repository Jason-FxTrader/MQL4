   ...
   for(shift=Bars-ExtDepth; shift>=0; shift--)
     {
      ...
      if(val==lastlow) val=0.0;
        ...
        { 
         lastlow=val; 
         if((Low[shift]-val)>(ExtDeviation*pipMultiplier*Point)) val=0.0;
         else
           {
      ...
      if(val==lasthigh) val=0.0;
        ...
        {
         lasthigh=val;
         if((val-High[shift])>(ExtDeviation*pipMultiplier*Point)) val=0.0;
         else
           {
            for(back=1; back<=ExtBackstep; back++)
              {
