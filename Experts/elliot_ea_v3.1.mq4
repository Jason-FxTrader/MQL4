...
extern int    LotLimit=1000; // use 500 for MINIS 
...
extern bool UseTick = false;
extern int  shift=0;

datetime TimeBarNow = D'0000.00.00', TimeBarPrev = D'0000.00.00';
...
double dielliot0=iCustom(NULL,0,"Elliott Wave Oscillator", 0,shift+1);
double dielliot1=iCustom(NULL,0,"Elliott Wave Oscillator", 0,shift+0);
double dielliot2=iCustom(NULL,0,"Elliott Wave Oscillator", 0,shift+1);
double dielliot3=iCustom(NULL,0,"Elliott Wave Oscillator", 0,shift+0);
double Juice = iCustom(NULL, 0, "Juice", 0,shift+0);


   if(AccountFreeMargin() < (LotLimit*dLots)){
   ...
   if (!ExistPositions()){
      ...
      if (lFlagBuyOpen){
         if(!UseTick) {
            if(TimeBarNow == TimeBarPrev) return(0);
            else {
               OpenBuy();
               TimeBarPrev = TimeBarNow;}
            }    
            if(UseTick) {OpenBuy();return(0);}
      }
