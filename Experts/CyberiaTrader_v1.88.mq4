...
//| emka207 a.k.a p0p3ye a.k.a. Johan Mustika Kartiwa -- johanmk.com                   |                                                                                   |
//|                                                                                    |
//+------------------------------------------------------------------------------------+
       ...
       if(OrderType()==OP_BUY)               // If the obtained order to the acquisition of the currency
         {
           ...
           if((OrderOpenPrice() < Bid) &&  BuySucPossibilityMid == 0 && SellSucPossibilityMid == 0)
            ...
            // Closing on Take Profit Timer timed-out
           if (TakePIPCountDown <= 0)
             {
               OrderClose(OrderTicket(), OrderLots(), Bid , SlipPage, Violet); // Close the order
               return (0);
             }
...
int VerbiageAndTimeCheck() {
   ...
   string comment_line="", comment_time="", comment_ver="", comment_PIPTimer="";
   string sp         = "------------------------------\n";
   comment_ver=StringConcatenate(SystemName," v. ",version,"\n");
    ...
    if (EnablePIPTimer == True && BestPIPProfit != -99999999){
        comment_PIPTimer = StringConcatenate("Best PIP: ", BestPIPProfit, ", PIP Timer: ", TakePIPCountDown);
        if (PIPTimerType == 0){
            comment_PIPTimer = StringConcatenate(comment_PIPTimer, " seconds.");
        }
        else{
            comment_PIPTimer = StringConcatenate(comment_PIPTimer, " ticks.");
        }
    }
  
   comment_line = comment_ver + sp + comment_time + comment_PIPTimer;
...
{
   if(FoundOpenedOrder == True && EnablePIPTimer == True)  // If there is the open order on this currency
   {
      if (LastOrderTicket != OrderTicket())
      {
         BestPIPProfit = -99999999;
         TakePIPCountDown = TakePIPTimer;
         LastCountDownResetTime = CurTime();
         LastOrderTicket = OrderTicket();
      }
   
      // Alert(Symbol(), " ", OrderOpenPrice(), " ", Bid, " ", Ask);
   
      double currentPIP = 0;
      if (OrderType() == OP_SELL)
      {
         currentPIP = (OrderOpenPrice()-Ask) / Point;
      }
      else if (OrderType() == OP_BUY)
      {
         currentPIP = (Bid-OrderOpenPrice()) / Point;
      }
      
      if (currentPIP > BestPIPProfit)
      {
         BestPIPProfit = currentPIP;
         TakePIPCountDown = TakePIPTimer;
         LastCountDownResetTime = CurTime();
      }
      else if (currentPIP >= MinPIPProfit)
      {
         if (PIPTimerType == 0)     // Seconds count
         {
            TakePIPCountDown = TakePIPTimer - (CurTime() - LastCountDownResetTime);
         }
         else
         {                          // tick count
            TakePIPCountDown--;
