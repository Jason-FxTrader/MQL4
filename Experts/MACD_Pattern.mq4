...
extern int minstoploss = 15;
extern double mLots = 0.1;
...
{
 //For extra digits
 if ((Point == 0.00001) || (Point == 0.001)) minstoploss = minstoploss * 10;
 return(0);
}
...
int start()
  {   
      ...
      if (Year()>3008 )  {Comment("Время работы эксперта истекло. FORTRADER.RU");return(0); }
...
void AOPattern6(int countbars,int maxbars,int minbars,double FastEMA,double SlowEMA,double maxur,double minur,int stoplossbars, int otstup,int takeprofitbars)
{  double sl;
   ...
   {sl=StopLoss(0,stoplossbars,otstup);if(sl<Bid){sl=sl+10*Point;}
     OrderSend(Symbol(),OP_SELL,Lots,Bid,3,sl,TakeProfit(0,takeprofitbars),"MacdPattern",maxdrow,0,Red);
   ...
   {sl=StopLoss(1,stoplossbars,otstup);if(sl>Ask){sl=sl-10*Point;}
      OrderSend(Symbol(),OP_BUY,Lots,Ask,3,sl,TakeProfit(1,takeprofitbars),"MacdPattern",maxdrow,0,Red);
      barnumms=0;
      aop_okbuy=0;
      nummodb=0;
      stop=0;
...
void AOPattern5(double FastEMA,double SlowEMA,double maxur,double minur,int stoplossbars, int otstup,int takeprofitbars)
{  double sl;
   //загружаем индикаторы
   double macdcurr =iMACD(NULL,0,FastEMA,SlowEMA,1,PRICE_CLOSE,MODE_MAIN,1);
   double macdlast =iMACD(NULL,0,FastEMA,SlowEMA,1,PRICE_CLOSE,MODE_MAIN,2);
...
double max14,min14;
void AOPattern4(double FastEMA,double SlowEMA,double in_maxur4,double in_minur4,int stoplossbars, int otstup,int takeprofitbars)
{  double sl;
   //загружаем индикаторы
   double macdcurr =iMACD(NULL,0,FastEMA,SlowEMA,1,PRICE_CLOSE,MODE_MAIN,1);
   double macdlast =iMACD(NULL,0,FastEMA,SlowEMA,1,PRICE_CLOSE,MODE_MAIN,2);
   ...
   if(macdcurr>in_maxur4 && macdcurr<macdlast && macdlast>macdlast3 &&  stops4==0){max14=macdlast;stops4=1;}
   if(macdcurr<in_maxur4 ){stops4=0;max14=0;}
   if(stops4==1 && macdcurr>in_maxur4&& macdcurr<macdlast && macdlast>macdlast3 && macdlast<max14 ){aop_oksell4=1;} 
   if(macdcurr<in_maxur4 ){aop_oksell4=0;}
   ...
   if(macdcurr<in_minur4 && macdcurr>macdlast && macdlast<macdlast3 &&  sstop4==0){min14=macdlast;sstop4=1;}
   if(macdcurr>in_minur4 ){sstop4=0;min14=0;}
   if(sstop4==1 && macdcurr<in_minur4&& macdcurr>macdlast && macdlast<macdlast3 && macdlast>min14 ){aop_okbuy4=1;} 
   if(macdcurr>in_maxur4 ){aop_okbuy4=0;}
...
void AOPattern3(double FastEMA,double SlowEMA,double in_maxur3,double in_minur3,int stoplossbars, int otstup,int takeprofitbars)
{double sl;
   //загружаем индикаторы
   double macdcurr =iMACD(NULL,0,FastEMA,SlowEMA,1,PRICE_CLOSE,MODE_MAIN,1);
   double macdlast =iMACD(NULL,0,FastEMA,SlowEMA,1,PRICE_CLOSE,MODE_MAIN,2);
   ...
   if(max13>0 &&macdcurr<in_maxur3){stops3=1;}
   
   if(macdcurr<maxur13 ){stops3=0;max13=0;S3=0;}
   

    if(stops3==1 && macdcurr>in_maxur3&& macdcurr<macdlast && macdlast>macdlast3 && macdlast>max13 && macdlast>max23 && stops13==0){max23=macdlast;} 
    
    if(max23>0 &&macdcurr<in_maxur3){stops13=1;}
   
   if(macdcurr<maxur13 ){stops13=0;max23=0;}
   
   if(stops13==1 && macdcurr<in_maxur3&& macdlast<in_maxur3&& macdlast3<in_maxur3&&  macdcurr<macdlast && macdlast>macdlast3 && macdlast<max23 && aop_oksell3==0){max33=macdlast;aop_oksell3=1;} 
   ...
   if(macdcurr<in_minur3){bS3=1;}
    
   if(bS3==1 && macdcurr>macdlast && macdlast<macdlast3 && macdlast<min13 && sstops3==0){min13=macdlast;}
   
   if(min13<0 &&macdcurr>in_minur3){sstops3=1;bS3=0;}
   
   if(macdcurr>minur13 ){sstops3=0;min13=0;bS3=0;}
   
    if(sstops3==1 && macdcurr<in_maxur3&& macdcurr>macdlast && macdlast<macdlast3 && macdlast<min13 && macdlast<min23 && sstops13==0){min23=macdlast;} 
    
    if(min23<0 &&macdcurr>in_minur3){sstops13=1;sstops3=0;}
   
   if(macdcurr>minur13 ){sstops13=0;min23=0;}
   
   if(sstops13==1 && macdcurr>in_minur3&& macdlast>in_minur3&& macdlast3>in_minur3&&  macdcurr>macdlast && macdlast<macdlast3 && macdlast>min23 && aop_okbuy3==0){min33=macdlast;aop_okbuy3=1;sstops13=0;} 
...
void AOPattern2(double FastEMA,double SlowEMA,double in_maxur2,double in_minur2,int stoplossbars, int otstup,int takeprofitbars)
{double sl;
   //загружаем индикаторы
   double macdcurr =iMACD(NULL,0,FastEMA,SlowEMA,1,PRICE_CLOSE,MODE_MAIN,1);
   double macdlast =iMACD(NULL,0,FastEMA,SlowEMA,1,PRICE_CLOSE,MODE_MAIN,2);
   ...
   if(macdcurr>macdlast && macdlast<macdlast3 && aop_maxur2==1  && macdcurr>in_minur2 && macdcurr<0 && aop_oksell2==0)
   { 
...
value_curr2=MathAbs(macdcurr*10000);
 ...
 if(aop_oksell2==1  && macdcurr<macdlast && macdlast>macdlast3 && macdcurr<0 && value_min2<=value_curr2)
   ...
   if(macdcurr<in_maxur2  && macdcurr<macdlast && macdlast>macdlast3 && aop_minur2==1  && macdcurr>0)
   { 
...
void AOPattern1(double FastEMA,double SlowEMA,double in_maxur1,double in_minur1,int stoplossbars, int otstup,int takeprofitbars)
{double sl;
   //загружаем индикаторы
   double macdcurr =iMACD(NULL,0,FastEMA,SlowEMA,1,PRICE_CLOSE,MODE_MAIN,1);
   double macdlast =iMACD(NULL,0,FastEMA,SlowEMA,1,PRICE_CLOSE,MODE_MAIN,2);
   ...
   if(macdcurr>in_maxur1){aop_maxur1=1;}
   if(macdcurr<0){aop_maxur1=0;}
   if(macdcurr<in_maxur1 && macdcurr<macdlast && macdlast>macdlast3 && aop_maxur1==1 && macdcurr>0 && macdlast3<in_maxur1)
   { 
   aop_oksell1=1;
   }
   if(aop_oksell1==1 )
   ...
   if(macdcurr<in_minur1){aop_minur1=1;}
   if(macdcurr>0){aop_minur1=0;}
   if(macdcurr>in_minur1 && macdcurr<0 && macdcurr>macdlast && macdlast<macdlast3 && aop_minur1==1 && macdlast3>in_minur1 )
   { 
   aop_okbuy1=1;
   }
   if(aop_okbuy1==1 )
...
if (stoploss-Bid < minstoploss*Point) stoploss = Bid+minstoploss*Point;
 return(stoploss);
}
if(type==1)
{
  stoploss=Low[iLowest(NULL,0,MODE_LOW,stoplossbars,1)]-otstup*Point;
 if (Ask-stoploss < minstoploss*Point) stoploss = Ask-minstoploss*Point;
 return(stoploss);
}
return(-1); 
}
...
{ int in_x=0,in_stop=0;double takeprofit;
  
  if(type==0)
   {
   while(in_stop==0)
         {
           takeprofit =Low[iLowest(NULL,0,MODE_LOW,takeprofitbars,in_x)];
          if(takeprofit>Low[iLowest(NULL,0,MODE_LOW,takeprofitbars,in_x+takeprofitbars)])
            {
            takeprofit =Low[iLowest(NULL,0,MODE_LOW,takeprofitbars,in_x+takeprofitbars)];
            in_x=in_x+takeprofitbars;
            }
          else
            {
             if (Bid-takeprofit < minstoploss*Point) takeprofit = Bid-minstoploss*Point;
             in_stop=1;return(takeprofit);
            }
         }
   }
   ...
   while(in_stop==0)
         {
           takeprofit =High[iHighest(NULL,0,MODE_HIGH,takeprofitbars,in_x)];
          if(takeprofit<High[iHighest(NULL,0,MODE_HIGH,takeprofitbars,in_x+takeprofitbars)])
            {
            takeprofit =High[iHighest(NULL,0,MODE_HIGH,takeprofitbars,in_x+takeprofitbars)];
            in_x=in_x+takeprofitbars;
            }
          else
            {
             if (takeprofit-Ask < minstoploss*Point) takeprofit = Ask+minstoploss*Point;
             in_stop=1;return(takeprofit);
            }
         }
   }
...
void  ActivePosManager(int in_perema1, int in_perema2, int in_persma3, int in_perema4)
{string comment=maxdrow;int i;double lt;
   double ema1 =iMA(NULL,0,in_perema1,0,MODE_EMA,PRICE_CLOSE,1);
    double ema2 =iMA(NULL,0,in_perema2,0,MODE_EMA,PRICE_CLOSE,1);
     double sma1 =iMA(NULL,0,in_persma3,0,MODE_SMA,PRICE_CLOSE,1);
      double ema3 =iMA(NULL,0,in_perema4,0,MODE_EMA,PRICE_CLOSE,1);
      ...
      if (OrderSelect(i-1,SELECT_BY_POS)==true) 
        {                                       
              ...
              {  lt=NormalizeDouble(OrderLots()/3,2);if(lt<=0.01){lt=0.01;}
                 OrderClose(OrderTicket(),mLots,Bid,3,Violet); 
                 nummodb++;
         ...
         if (OrderSelect(i-1,SELECT_BY_POS)==true) 
            ...
            { 
             ...
             if(OrderType()==OP_SELL && OrderProfit()>5 && Close[1]<ema2 && nummods==0 && OrderSymbol()==Symbol()&&OrderMagicNumber()==maxdrow)
              {lt=NormalizeDouble(OrderLots()/3,2);if(lt<=0.01){lt=0.01;}
                 OrderClose(OrderTicket(),mLots,Ask,3,Violet); 
                 nummods++;
