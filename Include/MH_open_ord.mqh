...
//---------------------------------------------------------------------------- 3 --
   while(Mas_Tip[Tip]==0)                // Until they ..
     {                                   //.. succeed
      ...
      MN=(int)TimeCurrent();                  // Simple MagicNumber
      ...
      if(Tip==0) // Let's open a Buy
        {
         ...
         if((Ticket=OrderSend(_Symbol,OP_BUY,Lots_New,Ask,SLIPPAGE,SL,TAKEPROFIT,"",MN))==-1)
      ...
      if(Tip==1) // Let's open a Sell
        {
         ...
         if((Ticket=OrderSend(_Symbol,OP_SELL,Lots_New,Bid,SLIPPAGE,SL,TAKEPROFIT,"",MN))==-1)
            Print(__LINE__,Error_Get()," OrderSend()");
